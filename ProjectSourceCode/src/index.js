...
app.get('/removeAssignment', async (req, res) => { // 
  try {
    console.log(req.body.assignment); // store the input to search 
    const requestedUser = req.assignment;
    const user = await db.oneOrNone('DELETE FROM assignments WHERE name = $1 AND ', [assignment]);
    if(!user) {
      return res.redirect('/removeAssignment');
    }
